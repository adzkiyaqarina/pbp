...
class PembimbingController extends Controller
{
    public function index(Request $request){
        ...
        $data_dosen = Dosen::where('user_id',Auth::user()->id)->first();
        ...
        if($request->isMethod('POST')){
            ...
            if($page === 'kalender-akademik'){
                ...
                if($mode === 'new-irs'){
                    ...
                    $irs_data = Irs::find($kr);
                    if($irs_data)
                        $irs_data->update(['status_lock_irs' => 'locked']);
                    Notifikasi::create([
                        'user_id'=>Auth::user()->id,
                ...
                return redirect('/mahasiswa?page=irs');
            }else if($page === 'perwalian'){
                $mode = $request->input('mode');
                if($mode === 'open-block'){
                    $kr = $request->query('kr');
                    $irs_data = Irs::find($kr);
                    if($irs_data)
                        $irs_data->update(['status_lock_irs' => 'open']);
                    Notifikasi::create([
                        'user_id'=>Auth::user()->id,
                        'teks'=>'Akses irs berhasil dibuka!'
                    ]);
                    return redirect('/pembimbing?page=perwalian');
                }else if($mode === 'add-note'){
                    $kr = $request->query('kr');
                    $note = $request->input('note');
                    $irs_data = Irs::find($kr);
                    if($irs_data)
                        $irs_data->update(['catatan' => $note]);
                    Notifikasi::create([
                        'user_id'=>Auth::user()->id,
                        'teks'=>'Catatan berhasil disimpan!'
                    ]);
                    return redirect('/pembimbing?page=perwalian');
                    ...
                    $irs_data = Irs::find($kr);
                    if($irs_data)
                        $irs_data->update(['status'=>'approved','status_lock_irs'=>'locked']);
                    Notifikasi::create([
                        'user_id'=>Auth::user()->id,
                        'teks'=>'Irs berhasil disetujui!'
                    ]);
                    return redirect('/pembimbing?page=perwalian');
                }else if($mode === 'reject'){
                    $kr = $request->query('kr');
                    $irs_data = Irs::find($kr);
                    if($irs_data)
                        $irs_data->update(['status_lock_irs' => 'open','status'=>'rejected']);
                    Notifikasi::create([
                        'user_id'=>Auth::user()->id,
                        'teks'=>'Irs berhasil ditolak!'
                    ]);
                    return redirect('/pembimbing?page=perwalian');
                }else if($mode === 'add-matkul'){
                    $kr = $request->query('kr');
        ...
        // working on data irs
        $irs_mahasiswa = Irs::where('nip_dosen_wali',$data_dosen->nip)->get();
        foreach($irs_mahasiswa as $item){
            $mahasiswa_data_ = Mahasiswa::where('nim',$item->nim)->first();
            if($mahasiswa_data_)
                $item->mahasiswa = $mahasiswa_data_;
        }
        ...
        $jumlah_mahasiswa = Mahasiswa::where('wali_dosen',Auth::user()->id)->count();
        $jumlah_irs_disetujui = Irs::where('nip_dosen_wali',$data_dosen->nip)->where('status','approved')->count();

        $data_irs_reminder = Irs::where('nip_dosen_wali',$data_dosen->nip)->where('status','pending')->where('status_lock_irs','locked')->limit(5)->get();

        // $data_irs_reminder = Irs::where('nip_dosen_wali',$data_dosen->nip)->where('status','pending')->where('status_lock_irs','locked')->limit(5)->get();

        foreach($data_irs_reminder as $item){
            $mahasiswa_ = Mahasiswa::where('nim',$item->nim)->first();
            if($mahasiswa_)
                $item->mahasiswa = $mahasiswa_;
        }

        return view('pembimbing.pages.'.$page,[
            'user'=>$user,
            'page'=>$page,
            'ck'=>$ck,
            ...
            'jumlah_irs_disetujui' => $jumlah_irs_disetujui,
            'data_irs_reminder'=>$data_irs_reminder
        ]);
    }
}
