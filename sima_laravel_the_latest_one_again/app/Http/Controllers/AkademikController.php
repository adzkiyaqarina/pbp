...
class AkademikController extends Controller
{
    public function index(Request $request){
        ...
        if($request->isMethod('POST')){
            ...
            if($page === 'kalender-akademik'){
                ...
                if($mode === 'add'){
                    if($id){
                        $data = $request->all();
                        $data['id_alokasi'] = $data['kr'];
                        $data['id_ruang'] = $data['id_ruangan'];
                        AlokasiRuangItems::create($data);
                        return redirect('/akademik?page=alokasi-ruangan&id=1&kr='.$kr);
                    }else{
                    ...
                    if($id){
                        $id = $request->input('id');
                        $alokasi_ruang_item_ = AlokasiRuangItems::find($id);
                        if($alokasi_ruang_item_)
                            $alokasi_ruang_item_->delete();
                        return redirect('/akademik?page=alokasi-ruangan&id=1&kr='.$kr);
                    }else{
                        $id = $request->input('id');
                        ...
                        if($alokasi_ruang){
                            ...
                            AlokasiRuangItems::where('id_alokasi',$id)->delete();
        ...
        $daftar_ruangan = [];
        if($id){
            $daftar_ruangan = AlokasiRuangItems::where('id_alokasi',$kr)->get();
            foreach($daftar_ruangan as $item){
                $gedung_ = Gedung::find($item->id_gedung);
                if($gedung_)
                    $item->gedung = $gedung_;
                $ruangan_ = Ruangan::find($item->id_ruang);
                if($ruangan_)
