...
class JadwalController extends Controller
{
    ...
    {
        $kr = $request->query('kr');
        $jadwal_kuliah = Jadwal::find($kr);
        $jadwal_items = JadwalItems::where('id_jadwal',$kr)->get();
        ...
        foreach ($jadwal_items as $item) {
            ...
            $item->status = $jadwal_kuliah->status;
            $item->tahun_ajaran = $jadwal_kuliah->tahun_ajaran;
            $item->semester = $jadwal_kuliah->semester;
        }

        // echo json_encode([$jadwal_kuliah,$jadwal_items]);
        ...
        $events = $jadwal_items->map(function ($jadwal) {
